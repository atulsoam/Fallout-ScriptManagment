...
const AdminSendEmail = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      ccList: form.ccList ? form.ccList.split(',').map((email) => email.trim()) : [],
      attachments: [],
    };
    ...
    try {
      ...
      setMessage({ type: 'success', text: res.message || 'Email sent successfully.' });
      setForm({
        receiverList: '',
        ccList: '',
        recipientName: '',
  ...
  return (
    <div className="max-w-4xl mx-auto mt-8 px-6 py-8 bg-white rounded-lg shadow-md">
      <h2 className="text-2xl font-semibold mb-6 text-gray-800">📧 Send Email (Admin Only)</h2>

      <form onSubmit={handleSubmit} className="space-y-5">
        <div>
          <label className="block font-medium text-gray-700">To <span className="text-gray-500 text-sm">(comma separated)</span></label>
          <input
            type="text"
            name="receiverList"
        ...
        <div>
          <label className="block font-medium text-gray-700">CC <span className="text-gray-500 text-sm">(optional)</span></label>
          <input
            type="text"
            name="ccList"
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            <label className="block font-medium text-gray-700">Info Link <span className="text-gray-500 text-sm">(optional)</span></label>
            <input
              type="url"
              name="infoLink"
              placeholder="https://..."
          ...
          <div>
            <label className="block font-medium text-gray-700">Information Text <span className="text-gray-500 text-sm">(optional)</span></label>
            <input
              type="text"
              name="information"
        ...
        <div className="pt-4">
          <button
            type="submit"
            className="w-full md:w-auto bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-6 py-2 rounded shadow disabled:opacity-50"
            disabled={loading}
          >
